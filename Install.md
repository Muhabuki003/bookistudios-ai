# bookistudios AI Install

This file is for coding agents. If the bookistudios AI repository is not already cloned and open, clone `https://github.com/bytedance/bookistudios-ai.git` first, then continue from the repository root.

## Goal

Bootstrap a bookistudios AI local development workspace on the user's machine with the least risky path available.
...
- The bookistudios AI repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch bookistudios AI.
...
- If the current directory is not the bookistudios AI repository root, clone `https://github.com/bytedance/bookistudios-ai.git` if needed, then change into the repository root.
- Confirm the current directory is the bookistudios AI repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start bookistudios AI
