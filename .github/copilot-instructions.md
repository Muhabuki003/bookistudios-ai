...
- `backend/packages/harness/bookistudio/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/bookistudio/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/bookistudio/subagents/` - subagent registry/execution
- `backend/packages/harness/bookistudio/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`bookistudio.agents:make_lead_agent`)
