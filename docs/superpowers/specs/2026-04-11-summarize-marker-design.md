...
Full scan of `backend/.bookistudios-ai/data/deerflow.db` `run_events`:
...
- `run_events` seq=1 → original human `"写一份关于bookistudios-ai的详细技术报告"` ✓ (event store is fine)
...
1. **Synchronous path**. The upstream class has both `before_model` and `abefore_model`. Our wrapper only overrides the async variant. If any bookistudios-ai code path ever uses the sync flow, those summarizations won't be captured. Mitigation: also override `before_model` and use `dispatch_custom_event` (sync variant) with the same pattern.
