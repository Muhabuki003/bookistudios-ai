...
```bash
# 清除已有数据
rm -f backend/.bookistudios-ai/users.db
...
```bash
# 检查数据库
sqlite3 backend/.bookistudios-ai/users.db "SELECT email, password_hash FROM users LIMIT 3;"
```
...
```bash
ls -la backend/.bookistudios-ai/users.db
```
...
```bash
sqlite3 backend/.bookistudios-ai/users.db "PRAGMA journal_mode;"
```
...
# 检查 admin 数量
sqlite3 backend/.bookistudios-ai/users.db \
...
# 检查宿主机上的 users.db
ls -la ${DEER_FLOW_HOME:-backend/.bookistudios-ai}/users.db
sqlite3 ${DEER_FLOW_HOME:-backend/.bookistudios-ai}/users.db \
...
docker logs bookistudios-ai-gateway 2>&1 | grep -E "ChannelManager|channel" | head -10
```
...
ls -la ${DEER_FLOW_HOME:-backend/.bookistudios-ai}/admin_initial_credentials.txt
# 预期文件权限: -rw------- (0600)

cat ${DEER_FLOW_HOME:-backend/.bookistudios-ai}/admin_initial_credentials.txt
# 预期内容: email + password 行

# 容器日志只输出文件路径，不输出密码本身
docker logs bookistudios-ai-gateway 2>&1 | grep -E "Credentials written to|Admin account"
# 预期看到: "Credentials written to: /...../admin_initial_credentials.txt (mode 0600)"

# 反向验证: 日志里 NEVER 出现明文密码
docker logs bookistudios-ai-gateway 2>&1 | grep -iE "Password: .{15,}" && echo "FAIL: leaked" || echo "OK: not leaked"
```
...
# 确认 langgraph 容器不存在
docker ps --filter name=bookistudios-ai-langgraph --format '{{.Names}}' | wc -l
# 预期: 0
