...
Quick setup instructions for bookistudios AI.

## Configuration Setup

bookistudios AI uses a YAML configuration file that should be placed in the **project root directory**.

### Steps

1. **Navigate to project root**:
   ```bash
   cd /path/to/bookistudios-ai
   ```
...
3. **Edit configuration**:
   ...
   export DEER_FLOW_PROJECT_ROOT="/path/to/bookistudios-ai"
...
- **Location**: `config.yaml` should be in `bookistudios-ai/` (project root)
- **Git**: `config.yaml` is automatically ignored by git (contains secrets)
- **Runtime root**: Set `DEER_FLOW_PROJECT_ROOT` if bookistudios AI may start from outside the project root
- **Runtime data**: State defaults to `.bookistudios-ai` under the project root; set `DEER_FLOW_HOME` to move it
...
**Recommended**: Place `config.yaml` in project root (`bookistudios-ai/config.yaml`).
