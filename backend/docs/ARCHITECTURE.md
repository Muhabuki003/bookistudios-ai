...
This document provides a comprehensive overview of the bookistudios AI backend architecture.
...
- `threads.py` - `/api/threads/{id}` - Local bookistudios AI thread data cleanup after LangGraph deletion
- `artifacts.py` - `/api/threads/{id}/artifacts` - Artifact serving
- `suggestions.py` - `/api/threads/{id}/suggestions` - Follow-up suggestion generation

The web conversation delete flow is now split across both backend surfaces: LangGraph handles `DELETE /api/langgraph/threads/{thread_id}` for thread state, then the Gateway `threads.py` router removes bookistudios AI-managed filesystem data via `Paths.delete_thread_dir()`.
...
|-------------|---------------|
| `/mnt/user-data/workspace` | `backend/.bookistudios-ai/threads/{thread_id}/user-data/workspace` |
| `/mnt/user-data/uploads` | `backend/.bookistudios-ai/threads/{thread_id}/user-data/uploads` |
| `/mnt/user-data/outputs` | `backend/.bookistudios-ai/threads/{thread_id}/user-data/outputs` |
| `/mnt/skills` | `bookistudios-ai/skills/` |
...
2. Gateway receives file
   - Validates file
   - Stores in .bookistudios-ai/threads/{thread_id}/user-data/uploads/
...
3. Returns response
   {
     "files": [{
       "filename": "doc.pdf",
       "path": ".bookistudios-ai/.../uploads/doc.pdf",
...
3. Gateway removes local bookistudios AI-managed files
   - Deletes .bookistudios-ai/threads/{thread_id}/ recursively
