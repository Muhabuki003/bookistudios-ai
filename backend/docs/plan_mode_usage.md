...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in bookistudios AI 2.0.
...
bookistudios AI uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall bookistudios AI prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with bookistudios AI's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/bookistudios-ai/backend/packages/harness/deerflow/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom bookistudios AI-style prompts**
...
- Custom prompts emphasize the same principles as bookistudios AI's main system prompt (clarity, action-oriented, critical rules)
