"""bookistudios AIClient — Embedded Python client for bookistudios AI agent system.

Provides direct programmatic access to bookistudios AI's agent capabilities
...
class bookistudios AIClient:
    """Embedded Python client for bookistudios AI agent system.
    ...
    def _ai_tool_calls_event(msg_id: str | None, tool_calls, additional_kwargs: dict[str, Any] | None = None) -> "StreamEvent":
        ...
        data: dict[str, Any] = {
            ...
            "content": "",
            "id": msg_id,
            "tool_calls": bookistudios AIClient._serialize_tool_calls(tool_calls),
        }
        if additional_kwargs:
            data["additional_kwargs"] = additional_kwargs
        return StreamEvent(type="messages-tuple", data=data)
    ...
    def _tool_message_event(msg: ToolMessage) -> "StreamEvent":
        ...
        return StreamEvent(
            ...
            data={
                "type": "tool",
                "content": bookistudios AIClient._extract_text(msg.content),
                "name": msg.name,
    ...
    def _serialize_message(msg) -> dict:
        ...
        if isinstance(msg, AIMessage):
            ...
            if msg.tool_calls:
                d["tool_calls"] = bookistudios AIClient._serialize_tool_calls(msg.tool_calls)
            if getattr(msg, "usage_metadata", None):
                d["usage_metadata"] = msg.usage_metadata
            if additional_kwargs := bookistudios AIClient._serialize_additional_kwargs(msg):
    ...
    def stream(
        ...
        * ``StreamBridge`` is an asyncio-queue decoupling producers from
        ...
        So ``bookistudios AIClient.stream()`` is a parallel, sync, in-process
