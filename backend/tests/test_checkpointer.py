...
class TestGetCheckpointer:
    ...
    def test_sqlite_creates_parent_dir(self):
        ...
        .bookistudios-ai directory has not been created).
        """
...
# ---------------------------------------------------------------------------
# bookistudios AIClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """bookistudios AIClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            ...
            patch("deerflow.client.bookistudios AIClient._get_tools", return_value=[]),
        ):
            client = bookistudios AIClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = bookistudios AIClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)
