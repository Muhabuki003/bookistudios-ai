"""Tests for bookistudios AIClient."""
...
def client(mock_app_config, tmp_path):
    """Create a bookistudios AIClient with mocked config loading."""
...
class TestClientInit:
    ...
    def test_custom_params(self, mock_app_config):
        ...
        with patch("deerflow.client.get_app_config", return_value=mock_app_config):
            c = bookistudios AIClient(model_name="gpt-4", thinking_enabled=False, subagent_enabled=True, plan_mode=True, agent_name="test-agent", available_skills={"skill1", "skill2"}, middlewares=[mock_middleware])
    ...
    def test_invalid_agent_name(self, mock_app_config):
        with patch("deerflow.client.get_app_config", return_value=mock_app_config):
            with pytest.raises(ValueError, match="Invalid agent name"):
                bookistudios AIClient(agent_name="invalid name with spaces!")
            with pytest.raises(ValueError, match="Invalid agent name"):
                bookistudios AIClient(agent_name="../path/traversal")
...
class TestStream:
    ...
    def test_custom_events_are_forwarded(self, client):
        ...
        # see bookistudios AIClient.stream() docstring and GitHub issue #1969.
    ...
    def test_messages_mode_emits_token_deltas(self, client):
        ...
        Regression for bytedance/bookistudios-ai#1969 — before the fix the client
...
class TestExtractText:
    def test_string(self):
        assert bookistudios AIClient._extract_text("hello") == "hello"
    ...
    def test_list_text_blocks(self):
        ...
        assert bookistudios AIClient._extract_text(content) == "first\nsecond"

    def test_list_plain_strings(self):
        assert bookistudios AIClient._extract_text(["a", "b"]) == "a\nb"

    def test_empty_list(self):
        assert bookistudios AIClient._extract_text([]) == ""

    def test_other_type(self):
        assert bookistudios AIClient._extract_text(42) == "42"
...
class TestMemoryManagement:
    ...
    def test_get_memory_config(self, client):
        config = MagicMock()
        config.enabled = True
        config.storage_path = ".bookistudios-ai/memory.json"
...
class TestGatewayConformance:
    """Validate that bookistudios AIClient return dicts conform to Gateway Pydantic response models.
    ...
    def test_get_memory_config(self, client):
        ...
        mem_cfg.storage_path = ".bookistudios-ai/memory.json"
...
class TestAtomicWriteJson:
    def test_temp_file_cleaned_on_serialization_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with pytest.raises(TypeError):
                bookistudios AIClient._atomic_write_json(target, bad_data)
    ...
    def test_happy_path_writes_atomically(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bookistudios AIClient._atomic_write_json(target, data)
...
class TestSerializeMessage:
    def test_system_message(self):
        ...
        result = bookistudios AIClient._serialize_message(msg)
