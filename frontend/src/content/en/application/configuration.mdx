---
title: Configuration
description: bookistudios AI App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
bookistudios AI App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
models:
  - name: deepseek-v3
    use: bookistudio.models.patched_deepseek:PatchedChatDeepSeek
...
Install Ollama provider: `cd backend && uv add 'bookistudio-harness[ollama]'`
...
```yaml
sandbox:
  use: bookistudio.sandbox.local:LocalSandboxProvider
...
```yaml
sandbox:
  use: bookistudio.community.aio_sandbox:AioSandboxProvider
  image: enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest
  replicas: 3
  idle_timeout: 600
```

Install: `cd backend && uv add 'bookistudio-harness[aio-sandbox]'`
...
tools:
  ...
  - use: bookistudio.community.ddg_search.tools:web_search_tool # default, no key required
  # - use: bookistudio.community.tavily.tools:web_search_tool
  #   api_key: $TAVILY_API_KEY

  # Web fetch (choose one)
  - use: bookistudio.community.jina_ai.tools:web_fetch_tool

  # Image search
  - use: bookistudio.community.image_search.tools:image_search_tool

  # File operations
  - use: bookistudio.sandbox.tools:ls_tool
  - use: bookistudio.sandbox.tools:read_file_tool
  - use: bookistudio.sandbox.tools:glob_tool
  - use: bookistudio.sandbox.tools:grep_tool
  - use: bookistudio.sandbox.tools:write_file_tool
  - use: bookistudio.sandbox.tools:str_replace_tool
  - use: bookistudio.sandbox.tools:bash_tool
```

### Thread state persistence (checkpointer)

By default, bookistudios AI uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.bookistudios-ai/
```
...
checkpointer:
  type: postgres
  connection_string: postgresql://user:password@localhost:5432/bookistudio
```
