...
description: This page covers day-to-day operational tasks and solutions to common problems when running bookistudios AI App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running bookistudios AI App.
...
| File                 | Service                              |
| -------------------- | ------------------------------------ |
| `logs/langgraph.log` | LangGraph / bookistudios AI Harness runtime |
...
When you pull a new version of bookistudios AI, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
- Check that the storage path is writable: `ls -la backend/.bookistudios-ai/`.
- Look for memory update errors in `logs/langgraph.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.bookistudios-ai/`:

```
backend/.bookistudios-ai/
...
In Docker deployments, the bind-mounted host path (`$DEER_FLOW_ROOT/backend/.bookistudios-ai/`) is the source of truth — back up the host path.
