...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. bookistudios AI provides built-in
...
bookistudios AI organizes tools into four categories:
...
tools:
  ...
  - use: bookistudio.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
```yaml
tools:
  - use: bookistudio.community.ddg_search.tools:web_search_tool
```
...
Install: `cd backend && uv add 'bookistudio-harness[tavily]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: bookistudio.community.exa.tools:web_search_tool
    api_key: $EXA_API_KEY
```
Semantic search with neural retrieval. Requires an [Exa](https://exa.ai) API key.

Install: `cd backend && uv add 'bookistudio-harness[exa]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: bookistudio.community.infoquest.tools:web_search_tool
...
```yaml
tools:
  - use: bookistudio.community.firecrawl.tools:web_search_tool
    api_key: $FIRECRAWL_API_KEY
```
Firecrawl-powered search and crawl. Requires a [Firecrawl](https://firecrawl.dev) API key.

Install: `cd backend && uv add 'bookistudio-harness[firecrawl]'`
...
```yaml
tools:
  - use: bookistudio.community.exa.tools:web_fetch_tool
    api_key: $EXA_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: bookistudio.community.infoquest.tools:web_fetch_tool
    api_key: $INFOQUEST_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: bookistudio.community.firecrawl.tools:web_fetch_tool
    api_key: $FIRECRAWL_API_KEY
```
  </Tabs.Tab>
</Tabs>
...
tools:
  ...
  # Or use InfoQuest:
  # - use: bookistudio.community.infoquest.tools:image_search_tool
  #   api_key: $INFOQUEST_API_KEY
```
