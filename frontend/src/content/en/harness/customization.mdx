---
title: Customization
description: bookistudios AI's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  bookistudios AI is designed to be adapted. You can extend agent behavior by writing
...
bookistudios AI's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
from bookistudio.agents.thread_state import ThreadState
...
```python
from bookistudio.sandbox.sandbox_provider import SandboxProvider
from bookistudio.sandbox.sandbox import Sandbox
...
```python
from bookistudio.agents.memory.storage import MemoryStorage
...
For custom backends, implement the LangGraph `BaseCheckpointSaver` interface and configure it programmatically when initializing the `bookistudios AIClient`.
...
guardrails:
  enabled: true
  provider:
    use: bookistudio.guardrails.builtin:AllowlistProvider
    config:
