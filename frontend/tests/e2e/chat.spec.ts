...
test.describe("Chat workspace", () => {
  ...
  test("can type a message in the input box", async ({ page }) => {
    ...
    await textarea.fill("Hello, bookistudios AI!");
    await expect(textarea).toHaveValue("Hello, bookistudios AI!");
  });
  ...
  test("sending a message triggers API call and shows response", async ({
    ...
    await expect(page.getByText("Hello from bookistudios AI!")).toBeVisible({
      timeout: 10_000,
    });
  });
});
